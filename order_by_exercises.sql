...
# -Modify your first query to order by first name. The first result should be Irena Reutenauer and the
# last result should be Vidya Simmen.
SELECT * FROM employees
WHERE first_name IN ('Irena', 'Vidya', 'Maya')
ORDER BY first_name;

# -Update the query to order by first name and then last name. The first result should now be Irena Acton
